...
async function search() {
  ...
  try {
    ...
    error.value = 'No se pudo completar la búsqueda. Verifica que ZincSearch esté corriendo.'
    hits.value = []
    total.value = 0
  } finally {
    loading.value = false
...
<template>
  <div class="flex h-full flex-col bg-slate-100 text-slate-900">
    <header class="border-b border-slate-200 bg-white">
      <div class="mx-auto flex max-w-7xl flex-col gap-4 px-6 py-5">
        <div class="flex items-baseline gap-3">
          <h1 class="text-2xl font-semibold tracking-tight">
            Yana<span class="text-indigo-600">killa</span>
          </h1>
          <span class="text-sm text-slate-500">buscador de correos</span>
        </div>
        ...
        <div class="flex flex-col gap-3 sm:flex-row">
          <div class="relative flex-1">
            ...
            <input
              ...
              class="w-full rounded-lg border border-slate-300 bg-white py-3 pl-12 pr-4 text-base outline-none transition focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200" />
          </div>
    ...
    <main class="mx-auto flex w-full max-w-7xl flex-1 gap-4 overflow-hidden px-6 py-5">
      <section class="flex w-2/5 min-w-80 flex-col overflow-hidden rounded-xl border border-slate-200 bg-white">
        ...
        <ul class="flex-1 divide-y divide-slate-100 overflow-y-auto">
          <li v-for="hit in hits" :key="hit.id">
            <button
              ...
              :class="[
                ...
                selected?.id === hit.id ? 'bg-indigo-50' : 'hover:bg-slate-50',
              ]">
              <span class="min-w-0">
          ...
          <li v-if="!hits.length && !loading" class="px-4 py-10 text-center text-sm text-slate-400">
      ...
      <section class="flex flex-1 flex-col overflow-hidden rounded-xl border border-slate-200 bg-white">
